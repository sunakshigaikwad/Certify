  ...
  const carouselSlides = [
    {
      ...
      visual: (
        <div className="bg-slate-950 text-emerald-400 p-5 rounded-lg border border-slate-900 font-mono text-[11px] space-y-2.5 shadow-lg text-left">
    ...
    },
    {
      ...
      visual: (
        <div className="bg-slate-950 border border-slate-900 rounded-lg p-5 shadow-lg space-y-3 text-left">
          <div className="flex items-center space-x-2 bg-emerald-955/40 text-emerald-400 text-[10px] px-2 py-1 rounded font-bold border border-emerald-900/50">
          ...
          <div className="space-y-1.5 text-[10px] text-slate-400">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-slate-200">4/5 Stars</span>
            </div>
            <div className="flex justify-between">
              <span>Communication & Teamwork:</span>
              <span className="font-bold text-slate-200">5/5 Stars</span>
            </div>
          </div>
        </div>
      )
    },
    {
      ...
      visual: (
        <div className="bg-emerald-955/30 border border-emerald-900/50 rounded-lg p-6 flex flex-col items-center justify-center text-center space-y-3 shadow-inner">
          <div className="h-16 w-16 bg-slate-900 border-2 border-emerald-600 rounded p-1 flex items-center justify-center shadow">
            <div className="w-full h-full bg-emerald-500/80 rounded"></div>
          </div>
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-emerald-400 block">SCAN TO VERIFY ORIGIN</span>
            <p className="text-[9px] text-emerald-500 font-mono">http://localhost:5173/verify/CP-9814</p>
          </div>
        </div>
      )
    },
        ...
        <div className="bg-slate-955 border border-slate-900 rounded-lg p-5 shadow-lg space-y-2 text-left">
          ...
          <div className="space-y-2">
            {[
              ...
              { role: 'Team Evaluator', desc: 'Pre-filled AI rating & decisions', color: 'bg-emerald-955/30 text-emerald-300 border-emerald-900/40' },
              { role: 'Employee', desc: 'Submit letter scans & track status', color: 'bg-yellow-955/20 text-yellow-300 border-yellow-900/40' }
            ].map(r => (
  ...
  return (
    <div className="bg-slate-800 min-h-screen flex flex-col font-sans text-slate-100 selection:bg-emerald-500/20 selection:text-emerald-350">
      ...
      <header className="border-b border-slate-900 py-4 px-8 bg-slate-900/95 backdrop-blur-md fixed top-0 left-0 right-0 z-50 flex items-center justify-between shadow-xl transition-all duration-200">
        ...
        <div className="flex items-center space-x-6">
          <nav className="hidden md:flex items-center space-x-6">
            <Link to="/verify" className="text-slate-300 hover:text-emerald-400 font-bold text-sm transition-colors">Verifier Portal</Link>
            <Link to="/login" className="text-slate-300 hover:text-emerald-400 font-bold text-sm transition-colors">Evaluator Portal</Link>
            <div className="h-4 w-px bg-slate-800"></div>
            <Link to="/register" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2 px-5.5 rounded-full text-sm transition-all duration-150 shadow hover:shadow-md border border-emerald-550/20">
              Register
            </Link>
          </nav>
          ...
          <div className="flex items-center space-x-2 bg-emerald-955/65 border border-emerald-900/50 rounded-full px-3 py-1 shadow-sm shrink-0">
            ...
            <span className="text-[10px] text-emerald-300 font-extrabold tracking-wide uppercase hidden sm:inline">Polygon Nodes Operational</span>
            <span className="text-[10px] text-emerald-300 font-extrabold tracking-wide uppercase sm:hidden">Online</span>
          </div>
        </div>
      </header>
      ...
      <section className="relative pt-44 pb-32 px-6 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 border-b border-slate-800 overflow-hidden">
        ...
        <div className="absolute top-24 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-gradient-to-r from-emerald-900/10 to-teal-900/10 rounded-full blur-3xl pointer-events-none z-0"></div>
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10 space-y-6">
          <motion.div 
            ...
            className="inline-flex items-center space-x-1.5 bg-emerald-955/70 border border-emerald-900/50 text-emerald-300 text-[10.5px] font-black px-4 py-1.5 rounded-full uppercase tracking-wider shadow-sm animate-pulse"
          >
          ...
          <motion.p 
            ...
            className="text-base sm:text-lg md:text-xl text-slate-300 leading-relaxed max-w-2xl mx-auto font-medium"
          >
          ...
          >
            <Link to="/register" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 px-8 rounded-full text-sm transition-all duration-150 shadow-md hover:shadow-lg text-center border border-emerald-550/20">
              Register Now
            </Link>
            <Link to="/verify" className="bg-slate-900/60 hover:bg-slate-900 text-slate-200 border border-slate-800 font-bold py-3.5 px-8 rounded-full text-sm transition-all duration-150 shadow-sm text-center">
              Verify Certificate
            </Link>
          </motion.div>
        </div>
          ...
          >
            ...
            <div className="relative bg-slate-950 border border-slate-800 p-1.5 rounded-xl shadow-inner flex items-center gap-2">
              ...
              <input 
                ...
                className="w-full pl-1 pr-3 py-2.5 text-sm bg-transparent outline-none text-white placeholder-slate-500 font-medium"
              />
              <button 
                onClick={handleSearch}
                disabled={isSearching}
                className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-6 py-3 rounded-lg transition-colors shadow shrink-0 flex items-center gap-1.5"
              >
            ...
            <AnimatePresence mode="wait">
              {isSearching && (
                <motion.div 
                  ...
                  exit={{ opacity: 0 }}
                  className="mt-8 border border-dashed border-emerald-800 rounded-xl p-8 bg-emerald-950/20 text-center flex flex-col items-center justify-center space-y-4"
                >
              ...
              {searchResult && !isSearching && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="mt-8 bg-slate-950/60 border border-slate-850 rounded-xl p-5 md:p-6 space-y-4 shadow-xl text-left animate-fade-in"
                >
                  <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
                    ...
                    <span className="bg-emerald-955/80 text-emerald-350 text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase border border-emerald-900/65 shadow-sm">
                      ✓ Anchored
                    </span>
                  </div>
              ...
              {searchError && !isSearching && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="mt-8 bg-red-955/20 border border-red-900/30 rounded-xl p-5 text-center flex flex-col items-center space-y-2 text-xs"
                >
                  <XCircle className="h-8 w-8 text-red-500" />
                  <p className="font-black text-red-300">Hash Match Failed</p>
                  <p className="text-slate-400 font-medium leading-relaxed">{searchError}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-emerald-950 via-slate-900 to-emerald-950 text-white rounded-2xl p-6 md:p-8 flex flex-col justify-between md:col-span-2 shadow-xl border border-emerald-900/30 min-h-[300px] hover:shadow-2xl transition-all text-left">
              ...
              <div className="space-y-2">
                <span className="text-[10.5px] font-black text-emerald-400 uppercase tracking-widest block">Polygon Integration</span>
                <h3 className="text-2xl font-black">Smart Contract Anchoring</h3>
                <p className="text-slate-350 text-xs leading-relaxed max-w-md font-medium">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl hover:shadow-2xl hover:border-emerald-500/20 transition-all text-left">
              ...
              <div className="space-y-2">
                <span className="text-[10.5px] font-black text-emerald-450 uppercase tracking-widest block">Automated Parsing</span>
                <h3 className="text-lg font-black text-slate-150">AI Score Estimations</h3>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl hover:shadow-2xl hover:border-emerald-500/20 transition-all text-left">
              ...
              <div className="space-y-2">
                <span className="text-[10.5px] font-black text-emerald-455 uppercase tracking-widest block">Instant Scanner</span>
                <h3 className="text-lg font-black text-slate-150">Security QR Lookups</h3>
                <p className="text-slate-350 text-xs leading-relaxed font-semibold">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 flex flex-col justify-between md:col-span-2 shadow-xl hover:shadow-2xl hover:border-emerald-500/20 transition-all text-left">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <span className="bg-blue-955/40 text-blue-300 text-[10px] font-black px-2.5 py-0.5 rounded border border-blue-900/50 shadow-sm">HR ADMIN</span>
                  <span className="bg-emerald-955/20 text-emerald-300 text-[10px] font-black px-2.5 py-0.5 rounded border border-emerald-900/50 shadow-sm">EVALUATOR MANAGER</span>
                  <span className="bg-yellow-955/20 text-yellow-300 text-[10px] font-black px-2.5 py-0.5 rounded border border-yellow-900/50 shadow-sm">EMPLOYEE</span>
                </div>
              </div>
            </div>
            ...
            {[
              ...
              <div key={idx} className="bg-slate-850 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col justify-between relative hover:border-emerald-500/20 hover:shadow-2xl transition-all duration-150 text-left">
                ...
                <div className="mt-4">
                  ...
                  <p className="text-slate-400 text-xs leading-relaxed font-semibold mt-2">{step.desc}</p>
                </div>
                {idx < 3 && (
      ...
      <section className="py-24 px-8 bg-slate-850/50 border-b border-slate-800">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-2">
            ...
            <p className="text-slate-400 text-sm leading-relaxed font-semibold animate-pulse">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-8 lg:p-12 relative overflow-hidden max-w-4xl mx-auto min-h-[380px] flex flex-col justify-between text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center flex-1">
              ...
              <div className="md:col-span-7 space-y-4 text-left">
                <span className="text-xs font-bold text-emerald-450 uppercase tracking-wide block">
                ...
                <ul className="space-y-1.5 pt-2">
                  {carouselSlides[activeSlide].details.map((detail, idx) => (
                    <li key={idx} className="text-xs text-slate-350 flex items-center space-x-2 font-medium">
              ...
              return (
                ...
                >
                  <button 
                    ...
                    className="w-full py-4.5 px-6 flex items-center justify-between text-left text-sm md:text-base font-bold text-slate-100 hover:bg-slate-900/40 transition-colors"
                  >
                    <span>{faq.q}</span>
                    {isOpen ? <ChevronUp className="h-4 w-4 text-emerald-450 shrink-0 ml-4 animate-pulse" /> : <ChevronDown className="h-4 w-4 text-slate-500 shrink-0 ml-4" />}
                  </button>
      ...
      <section className="py-20 px-8 bg-gradient-to-b from-slate-800 to-slate-900 text-center relative overflow-hidden border-b border-slate-800">
        ...
        <div className="max-w-3xl mx-auto relative z-10 space-y-6">
          ...
          <div className="pt-2">
            <Link 
              to="/register" 
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 px-10 rounded-lg text-sm shadow-md hover:shadow-lg transition-all duration-150 inline-flex items-center space-x-2 border border-emerald-550/25"
            >
      ...
      <footer className="border-t border-slate-900 py-8 px-8 bg-slate-950 flex flex-col md:flex-row justify-between items-center text-sm text-slate-500 mt-auto">
        ...
        <div className="flex space-x-6 mt-4 md:mt-0 font-semibold text-slate-400">
          ...
          <Link to="/login" className="hover:text-emerald-400">HR Sign In</Link>
        </div>
      </footer>
    </div>
  );
